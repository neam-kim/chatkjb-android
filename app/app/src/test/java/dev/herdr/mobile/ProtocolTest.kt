...
class ProtocolTest {
   ...
   @Test fun buildsCloseImpactRequest() {
    ...
    @Test fun buildsExactQServantV8ClientOperations() {
        assertEquals(
            """{"t":"qservant_catalog","reqId":"q1"}""",
            ClientMsg.qservantCatalog("q1"),
        )
        assertEquals(
            """{"t":"qservant_submit","reqId":"q2","model":"live/model","effort":"high","audioMime":"audio/mp4","audioBase64":"YWI="}""",
            ClientMsg.qservantSubmit("q2", "live/model", "high", "YWI="),
        )
        assertEquals(
            """{"t":"qservant_status","reqId":"q3","jobId":"job-9"}""",
            ClientMsg.qservantStatus("q3", "job-9"),
        )
        assertEquals(
            """{"t":"qservant_cancel","reqId":"q4","jobId":"job-9"}""",
            ClientMsg.qservantCancel("q4", "job-9"),
        )
    }

    @Test fun parsesQServantCatalogNullQuotaUnknownFieldsAndDefaults() {
        val f = parseServerFrame(
            """{"t":"qservant_catalog_result","reqId":"q1","future":true,"models":[{"id":"live/a","label":"A","efforts":["low","high"],"quota":null,"unknown":1},{"id":"live/b","label":"B","efforts":[],"defaultEffort":"medium","quota":{"used":0.42,"label":"ok","extra":"x"}}],"defaultModel":"live/a","updatedAt":"2026-08-31T00:00:00Z"}""",
        )
        assertTrue(f is ServerFrame.QServantCatalogResult)
        val catalog = f as ServerFrame.QServantCatalogResult
        assertEquals("q1", catalog.reqId)
        assertEquals("live/a", catalog.defaultModel)
        assertNull(catalog.defaultEffort)
        assertEquals("2026-08-31T00:00:00Z", catalog.updatedAt)
        assertEquals(2, catalog.models.size)
        val a = catalog.models[0]
        assertEquals("live/a", a.id)
        assertEquals("A", a.label)
        assertEquals(listOf("low", "high"), a.efforts)
        assertNull(a.defaultEffort)
        assertNull(a.quota)
        val b = catalog.models[1]
        assertEquals("medium", b.defaultEffort)
        assertEquals(0.42, b.quota?.used)
        assertEquals("ok", b.quota?.label)
        val missing = parseServerFrame("""{"t":"qservant_catalog_result","reqId":"q2","models":[]}""") as ServerFrame.QServantCatalogResult
        assertNull(missing.defaultModel)
        assertNull(missing.updatedAt)
        assertTrue(missing.models.isEmpty())
    }

    @Test fun parsesQServantJobReportAndErrorFixtures() {
        val job = parseServerFrame(
            """{"t":"qservant_job","reqId":"q9","extra":1,"job":{"jobId":"job-9","state":"completed","transcript":"안녕","report":{"request":"do","work":"did","verification":"ok","changes":["a.kt"],"commit":"abc","result":"done","success":true},"error":null}}""",
        )
        assertTrue(job is ServerFrame.QServantJobFrame)
        val jf = job as ServerFrame.QServantJobFrame
        assertEquals("q9", jf.reqId)
        assertEquals("job-9", jf.job.jobId)
        assertEquals("completed", jf.job.state)
        assertEquals("안녕", jf.job.transcript)
        assertNull(jf.job.error)
        val report = jf.job.report!!
        assertEquals("do", report.request)
        assertEquals("did", report.work)
        assertEquals("ok", report.verification)
        assertEquals(listOf("a.kt"), report.changes)
        assertEquals("abc", report.commit)
        assertEquals("done", report.result)
        assertTrue(report.success)
        val noReq = parseServerFrame("""{"t":"qservant_job","job":{"jobId":"job-1","state":"running"}}""") as ServerFrame.QServantJobFrame
        assertNull(noReq.reqId)
        assertEquals("running", noReq.job.state)
        val err = parseServerFrame("""{"t":"qservant_error","reqId":"q8","code":"space_missing","message":"herdr space 'Q Servant' not found","unknown":false}""") as ServerFrame.QServantError
        assertEquals("q8", err.reqId)
        assertNull(err.jobId)
        assertEquals("space_missing", err.code)
        assertEquals("herdr space 'Q Servant' not found", err.message)
        val errJob = parseServerFrame("""{"t":"qservant_error","reqId":"q7","jobId":"job-1","code":"stt_failed","message":"boom"}""") as ServerFrame.QServantError
        assertEquals("job-1", errJob.jobId)
    }

    @Test fun parsesQServantChangesStringAsSingleReportEntry() {
        val frame = parseServerFrame(
            """{"t":"qservant_job","job":{"jobId":"job-live","state":"completed","report":{"request":"do","work":"did","verification":"ok","changes":"no files changed","result":"done","success":true}}}""",
        ) as ServerFrame.QServantJobFrame
        assertEquals(listOf("no files changed"), frame.job.report?.changes)
    }
}
