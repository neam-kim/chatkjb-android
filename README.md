...
- **Email / KJBMail**: `dev.herdr.kjbmail:mail-host`의 Thunderbird Mail host를 통합해 `MailEntryActivity`를 실행합니다. KJBMail 소스는 `KJBMail/`에 일반 파일로 포함되어 있습니다.
...
KJBMail은 저장소 내부 composite build로 연결됩니다. WebView의 백그라운드 Web Push 지원은 일반 설치형 브라우저 PWA와 다를 수 있으므로, foreground agent control을 우선 보장합니다.

## 설치

### Android

요구사항: JDK 21, Android SDK (compileSdk 37), NDK 29.0.14206865. Relay는 [`0cv/herdr-mobile-relay`](https://github.com/0cv/herdr-mobile-relay)의 설치 지침을 따릅니다.
...
코드 책임 분리와 내부 소스 관리 방법은 [`docs/source-ownership.md`](docs/source-ownership.md), 가져온 커밋과 라이선스는 [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md)에 기록합니다.
